...
flowplayer(function (a, b) {
    ...
    if (true) d.logo && b.append(c("<a>", {
        ...
        var h = c("<a/>").attr("href", "http://flowplayer.org").appendTo(b),
            i = c(".fp-controls", b);
        b.bind("mouseenter mouseleave", function (b) {
            a.ready && h.toggle(b.type == "mouseenter")
        }), a.bind("progress unload", function (c) {
            c.type == "progress" && a.video.time < 8 && a.engine != "flash" && b.hasClass("is-mouseover") ? (h.show().css({
                position: "absolute",
                left: 6,
                bottom: i.height() + 12,
                zIndex: 99999,
                ...
                height: 20,
                cursor: "pointer",
                backgroundImage: "url(http://stream.flowplayer.org/logo.png)"
            }), a.load.ed = h.is(":visible")) : h.hide()
        })
    }
});
*/
